...
class ReplayBuffer:
    ...
    def add_path(self, path):
        ...
        if 'terminals' in path and path['terminals'].any():
        ...
        if 'terminals' in path and path['terminals'].any() and self.termination_penalty is not None:
    ...
    def finalize(self, bc_percentage=None):
        ## remove extra slots
        for key in self.keys + ['path_lengths']:
            self._dict[key] = self._dict[key][:self._count]
        # filt out bad trajectories if necessary
        if bc_percentage:
            avg_reward_list = []
            for _ in range(self._count):
                avg_reward_list.append(np.mean(self._dict['rewards']))
            sorted_idx = np.flip(np.argsort(avg_reward_list))
            keep_idx = sorted_idx[0: int(bc_percentage * self._count)]
            for k in self._dict.keys():
                self._dict[k] = self._dict[k][keep_idx]
            self._count = keep_idx.shape[0]
        self._add_attributes()
